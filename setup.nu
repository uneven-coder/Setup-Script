let default_mod = "MyModName"
let default_author = "Cratior"
let default_desc = "SFS mod with auto-generated structure"
...
let steam_root = (try {
    ...
    | find 'SteamPath'
    | first
    | parse -r 'SteamPath\s+REG_SZ\s+(?<path>.+)'
    | get path.0
    | str trim)
} catch {
    try {
        ...
        | find 'InstallPath'
        | first
        | parse -r 'InstallPath\s+REG_SZ\s+(?<path>.+)'
        | get path.0
        | str trim)
    } catch { "" }
})

let sfs_path = (if $steam_root != "" {
    $steam_root + '\steamapps\common\Spaceflight Simulator\Spaceflight Simulator Game\Mods'
} else {
    'C:\Program Files (x86)\Steam\steamapps\common\Spaceflight Simulator\Spaceflight Simulator Game\Mods'
})

print ("Detected Mods folder: " + $sfs_path)

let confirm = (input "Continue? (y/n): ")
if $confirm != "y" { error make { msg: "Cancelled" } }
...
open $csproj | str replace '</Project>' (
'  <PropertyGroup>
...
^dotnet sln $"($mod).sln" add $csproj
^dotnet build $csproj -c Debug

print "Build configured: DLL will auto-copy on msbuild"